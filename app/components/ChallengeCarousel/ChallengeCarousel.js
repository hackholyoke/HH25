...
const [emblaRef, emblaApi] = useEmblaCarousel(mergedOptions);
  ...
  const setTweenNodes = useCallback((emblaApi) => {
    tweenNodes.current = emblaApi.slideNodes().map((slideNode) => {
      return slideNode.querySelector('.c-card-container')
    })
  }, [])
  ...
  return (
    <div className="challenge-embla">
      <div className="challenge-embla__viewport" ref={emblaRef}>
        <div className="challenge-embla__container">
          {slides.map((slide, index) => (
            <div
            className=
            //'embla__slide--center'
            {`challenge-embla__slide ${index === activeIndex ? 'challenge-embla__slide--center' : ''}`}
            key={index}
          >
